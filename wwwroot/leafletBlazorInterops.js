...
window.leafletBlazor = {
    ...
    addMarkerCluster: function (mapId, markerCluster, objectReference) {
        let options = {
            ...createInteractiveLayer(markerCluster),
            showCoverageOnHover: markerCluster.showCoverageOnHover,
            zoomToBoundsOnClick: markerCluster.zoomToBoundsOnClick,
            spiderfyOnMaxZoom: markerCluster.spiderfyOnMaxZoom,
            removeOutsideVisibleBounds: markerCluster.removeOutsideVisibleBounds,
            animate: markerCluster.animate,
            animateAddingMarkers: markerCluster.animateAddingMarkers,
            disableClusteringAtZoom: markerCluster.disableClusteringAtZoom,
            maxClusterRadius: markerCluster.maxClusterRadius,
            polygonOptions: markerCluster.polygonOptions,
            singleMarkerMode: markerCluster.singleMarkerMode,
            spiderLegPolylineOptions: markerCluster.spiderLegPolylineOptions,
            spiderfyDistanceMultiplier: markerCluster.spiderfyDistanceMultiplier
        };
        const mkr = L.markerClusterGroup(options);
        connectMarkerClusterEvents(mkr, objectReference);
        for (let m = 0; m < markerCluster.markers.length; m++) {
            const myMarker = createMarker(mapId, markerCluster.markers[m], objectReference, true);
            markers.push(myMarker);
            //addMarkerClusterLayer(mapId, mkr, myMarker);
            mkr.addLayer(myMarker);
        }
        
        addLayer(mapId, mkr, objectReference);
        //mkr.addLayers(markerCluster.markers);
        
    },
    removeToClusterLayer: function (mapId, layerId, markerId) {
        ...
        for (var i = 0; i > markers.length; i++) {
            if (markers[i].options.id === markerId) {

                layers[mapId] = remainingLayers;

                layersToBeRemoved.forEach(m => m.removeLayer(markers[i]));
            }
        }
        //const layerToBeRemoveOnCluster = markers.filter(marker) => marker.id === markerId);
...
window.leafletMarkerLayer = {
    addMarker: function (mapId, marker, objectReference, isClustered) {
        createMarker(mapId, marker, objectReference, isClustered)
    },
...
function createMarker(mapId, marker, objectReference, isClustered) {
    var options = {
...
function connectMarkerClusterEvents(markerCluster, objectReference) {

    connectInteractiveLayerEvents(markerCluster, objectReference);

    //mapEvents(markerCluster, objectReference, {
    //    "move": "NotifyMove",
    //    "dragstart": "NotifyDragStart",
    //    "movestart": "NotifyMoveStart",
    //    "drag": "NotifyDrag",
