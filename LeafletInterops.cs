...
namespace BlazorLeaflet
{
    public static class LeafletInterops
    {
        ...
        private static readonly string _BaseObjectMarkerContainer = "window.leafletMarkerLayer";
        ...
        public static ValueTask AddLayer(IJSRuntime jsRuntime, string mapId, Layer layer)
        {
            return layer switch
            {
                ...
                Marker marker => jsRuntime.InvokeVoidAsync($"{_BaseObjectMarkerContainer}.addMarker", mapId, marker, CreateLayerReference(mapId, marker), false),
                ...
                MarkerCluster markerCluster => jsRuntime.InvokeVoidAsync($"{_BaseObjectContainer}.addMarkerCluster", mapId, markerCluster, CreateLayerReference(mapId, markerCluster)),
                _ => throw new NotImplementedException($"The layer {typeof(Layer).Name} has not been implemented."),
            };
        }

        public static async ValueTask RemoveToClusterLayer(IJSRuntime jsRuntime, string mapId, Layer layer)
        {
            
            await jsRuntime.InvokeVoidAsync($"{_BaseObjectContainer}.removeToClusterLayer", mapId, layer);
        }

        public static async ValueTask RemoveLayer(IJSRuntime jsRuntime, string mapId, Layer layer)
        {
            if(nameof(layer) == typeof(MarkerCluster).Name)
            {
                await jsRuntime.InvokeVoidAsync($"{_BaseObjectContainer}.removeToClusterLayer", mapId, layer.Id);

            }
            else
            {
                await jsRuntime.InvokeVoidAsync($"{_BaseObjectContainer}.removeLayer", mapId, layer.Id);
        ...
        public static ValueTask<LatLng> GetLatLng(IJSRuntime jsRuntime, string mapId) =>
            jsRuntime.InvokeAsync<LatLng>($"{_BaseObjectMarkerContainer}.getLatLng", mapId);
    }
}
